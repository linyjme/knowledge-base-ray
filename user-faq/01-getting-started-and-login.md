...
This procedure resets an **end-user** account. A forgotten **admin** account password is reset on the machine where the Raysync service is deployed. On Linux, run the binary **AdminUnlockTool**. On Windows, run the binary **AdminUnlockTool.exe**. Follow the operations that program presents. The user portal **Forget Password** flow does not reset the admin account.
